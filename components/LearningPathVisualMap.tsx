...
export function LearningPathVisualMap({
  ...
  useEffect(() => {
    ...
    const containerHeight = Math.max(minContainerHeight, learningPaths.length * pathVerticalSpacing + 200);
    const nodeWidth = 180;
    ...
    learningPaths.forEach((path, pathIndex) => {
      ...
      let courseWidth = Math.max(minCourseWidth, Math.min(maxCourseWidth, (availableWidth - (coursesCount - 1) * minSpacing) / coursesCount));
      let courseSpacing = Math.max(minSpacing, (availableWidth - coursesCount * courseWidth) / (coursesCount - 1));
  ...
  const getConnections = () => {
    ...
    nodes.forEach(node => {
      if (node.type === 'start') {
        ...
        const nextCourse = nodes.find(n => n.id === `course-${node.pathIndex}-${(node.courseIndex || 0) + 1}`);
        if (nextCourse) {
  ...
  return (
    <div ref={containerRef} className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 p-8">
      {/* Header */}
      ...
      {/* Flow Graph */}
      <div className="relative" style={{ height: `${Math.max(400, learningPaths.length * 150 + 200)}px` }}>
        {/* SVG for connections */}
        <svg
          ref={svgRef}
        ...
        >
          {nodes.length > 0 && getConnections().map((connection, index) => {
          ...
          {nodes.map((node) => {
              ...
              return (
                ...
                >
                  <div className="w-full h-full rounded-full bg-gradient-to-br from-green-400 to-green-600 flex items-center justify-center shadow-lg">
                    <span className="text-white font-bold text-sm">Αρχή</span>
                  </div>
                </motion.div>
              );
            }
            ...
            if (node.type === 'goal') {
              ...
              return (
                ...
                >
                  <div className="w-full h-full rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center shadow-xl">
                    <div className="text-center">
                      <span className="text-white font-bold text-sm">Στόχος</span>
                      <div className="text-white text-xs mt-1">Έτοιμος/η!</div>
                    </div>
                  </div>
                </motion.div>
              );
            ...
            if (node.type === 'course' && node.data) {
              ...
              return (
                ...
                >
                  <div
                    className={`w-full h-full rounded-xl border-2 p-3 transition-all duration-300 ${
                      isHighlighted
                        ? 'shadow-lg'
                        : 'shadow-md'
                    }`}
                  ...
                  >
                    <div className="h-full flex flex-col justify-between">
                      <div>
                        <h6 className="text-xs font-semibold text-gray-900 line-clamp-2">
                          {course.name}
                        </h6>
                  ...
                  <AnimatePresence>
                    {isHovered && (
                      <motion.div
                        className="absolute z-50 w-64 p-4 bg-white rounded-lg shadow-xl border border-gray-200"
                        ...
                        onMouseLeave={() => setHoveredNode(null)}
                      >
                        <h6 className="font-semibold text-sm text-gray-900 mb-2">{course.name}</h6>
                        <p className="text-xs text-gray-600 mb-2">{course.institution_name}</p>
                        {course.skills && course.skills.length > 0 && (
                          <div>
                            <p className="text-xs font-medium text-gray-700 mb-1">Δεξιότητες που καλύπτει:</p>
                            <div className="space-y-1">
                              {course.skills.slice(0, 3).map((skill) => (
                                <div
                                  key={skill.id}
                                  className={`text-xs px-2 py-1 rounded ${
                                    skill.is_essential ? 'bg-indigo-100 text-indigo-700' : 'bg-gray-100 text-gray-700'
                                  }`}
                                >
                                  {skill.preferred_label}
                                </div>
                              ...
                              {course.skills.length > 3 && (
                                <p className="text-xs text-gray-500">+{course.skills.length - 3} περισσότερες</p>
                              )}
                            </div>
                          </div>
                        )}
                        <Link
                          href={`/courses/${course.id}`}
                          className="inline-block mt-3 text-xs text-indigo-600 hover:text-indigo-800 font-medium"
                        >
                          Δείτε λεπτομέρειες →
                        </Link>
                      </motion.div>
      ...
      <div className="mt-8 p-4 bg-white/80 rounded-xl border border-gray-200">
        <h4 className="text-sm font-semibold text-gray-900 mb-3">Επιλέξτε διαδρομή για λεπτομέρειες:</h4>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-600 mt-1">
                  Καλύπτει {learningPaths[selectedPath].covered_essential_skills} από {essentialSkills.length} απαραίτητες δεξιότητες
                </p>
              </div>
            </div>
          </div>
